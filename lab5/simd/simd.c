...
void multiply_cache_unrolled_blocked_simd(int **a, int **b, int **c, int **b_transposed, int tiling_factor) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            b_transposed[j][i] = b[i][j];
        }
            ...
            for (int k = 0; k < N; k += tiling_factor) {
                for (int ii = i; ii < i + tiling_factor; ii++) {
                    for (int jj = j; jj < j + tiling_factor; jj++) {
                        ...
                        for (int kk = k; kk < k + tiling_factor; kk += 8) {
                            __m256i va = _mm256_loadu_si256((__m256i*)&a[ii][kk]);
                            __m256i vb = _mm256_loadu_si256((__m256i*)&b_transposed[jj][kk]);
...
void multiply_double_sumator_kahana_simd(double **d, double **e, long double **g) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            long double suma = 0.0;
            long double error = 0.0;
            for (int k = 0; k < N; k += 4) {
                __m256d va = _mm256_loadu_pd(&d[i][k]);
                __m256d vb = _mm256_set_pd(e[k + 3][j], e[k + 2][j], e[k + 1][j], e[k][j]);
                __m256d vmul = _mm256_mul_pd(va, vb);

                double temp[4];
                _mm256_storeu_pd(temp, vmul);
                for (int x = 0; x < 4; x++) {
                    long double y = temp[x] - error;
                    long double t = suma + y;
                    error = (t - suma) - y;
                    suma = t;
                }
...
void calculate_error_long(long double **reference, long double **result, 
                     long double *min_error, long double *max_error, long double *mean_error) {
    *max_error = 0.0;
    *min_error = LDBL_MAX;// Ustawienie na maksymalną wartość dla double
    ...
    if (count > 0) {
        ...
        *mean_error = 0.0; // Brak wartości do obliczenia średniego błędu
        *min_error = 0.0; // Ustawienie na 0, gdy brak wartości
        *max_error = 0.0; // Ustawienie na 0, gdy brak wartości
    }
}
...
int main() {
    ...
    long double min_error_simd, max_error_simd, mean_error_simd;
    
    unsigned long long time_int = 0, time_int_simd = 0, time_double = 0, time_double_simd = 0;
    ...
    long double **f = malloc(N * sizeof(long double*));
    ...
    for (int i = 0; i < N; i++) {
        ...
        f[i] = malloc(N * sizeof(long double));
    ...
    printf("Czas mnożenia int bez SIMD %llu cykli\n\n", end - start);
    time_int += (end - start);
    ...
    __asm__ __volatile__ ("rdtsc" : "=a"(lo), "=d"(hi));
    start = ((unsigned long long)hi << 32) | lo;
    multiply_cache_unrolled_blocked_simd(a, b, c, b_transposed, tiling_factor);
    __asm__ __volatile__ ("rdtscp" : "=a"(lo), "=d"(hi) : : "%rcx");
    __asm__ __volatile__ ("cpuid" : : : "%rax", "%rbx", "%rcx", "%rdx");
    end = ((unsigned long long)hi << 32) | lo;
    printf("Czas mnożenia int z SIMD %llu cykli\n\n", end - start);
    time_int_simd += (end - start);
    ...
    __asm__ __volatile__ ("rdtsc" : "=a"(lo), "=d"(hi));
    start = ((unsigned long long)hi << 32) | lo;
    multiply_double_sumator_kahana_simd(d, e, f);
    __asm__ __volatile__ ("rdtscp" : "=a"(lo), "=d"(hi) : : "%rcx");
    __asm__ __volatile__ ("cpuid" : : : "%rax", "%rbx", "%rcx", "%rdx");
    end = ((unsigned long long)hi << 32) | lo;
    printf("Czas mnożenia double_sorted_sumator_kahana_simd: %llu cykli\n", end - start);
    
    calculate_error_long(reference, g, &min_error_simd, &max_error_simd, &mean_error_simd);
    printf("Minimalny błąd względny: %Le\n", min_error_simd);
    printf("Maksymalny błąd względny: %Le\n", max_error_simd);
    printf("Średni błąd względny: %Le\n\n", mean_error_simd);
    time_double_simd += (end - start);
    
    min_error_sum += min_error_long;
    max_error_sum += max_error_long;
    mean_error_sum += mean_error_long;
    
    printf("Różnica minimalnego błędu: %Le\n", (min_error_simd - min_error_long));
    printf("Różnica maksymalnego błędu: %Le\n", (max_error_simd - max_error_long));
    printf("Różnica średniego błędu: %Le\n", (mean_error_simd - mean_error_long));
    
    reset(c, g);
    }
    
    printf("Średni czas mnożenia int bez SIMD %llu cykli\n\n", (time_int/100));
    printf("Średni czas mnożenia int z SIMD %llu cykli\n\n", (time_int_simd/100));
    
    printf("Średni czas mnożenia double_sorted_sumator_kahana: %llu cykli\n", (time_double/100));
    printf("Średni czas mnożenia double_sorted_sumator_kahana_simd: %llu cykli\n", (time_double_simd/100));
    ...
    for (int i = 0; i < N; i++) {
        ...
        free(b_transposed[i]);
        free(d[i]); free(e[i]);
        free(f[i]); free(g[i]); free(reference[i]);
    }
    free(a); free(b); free(c);
    free(b_transposed);
    free(d); free(e);
    free(f); free(g); free(reference);
